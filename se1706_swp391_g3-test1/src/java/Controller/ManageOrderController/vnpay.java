...
public class vnpay extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        System.out.println("dada: " + dao.getTotalCoursePrice(orderID).isEmpty());
        System.out.println("option:" + option);
        if (option.equals("1")) {
            total = dao.getTotalPrice(orderID);
            vnpayExecute(req, resp, total, orderID);
        } else if (option.equals("2")) {
            if (!dao.getTotalCoursePrice(orderID).isEmpty()) {

                total = dao.getTotalCoursePrice(orderID);
                vnpayExecute(req, resp, total, orderID);
            } else {
                com.google.gson.JsonObject job = new JsonObject();
                job.addProperty("code", "01");
                job.addProperty("message", "success");
                job.addProperty("data", Config.vnp_Returnurl + "?vnp_TransactionStatus=00");
                Gson gson = new Gson();
...
//        System.out.println("status:"+ tranStatus);
    }
    ...
    public void vnpayExecute(HttpServletRequest req, HttpServletResponse resp, String total, String orderID) throws ServletException, IOException {
        String vnp_Version = "2.1.0";
        String vnp_Command = "pay";
        String orderType = "sale";
        long amount = convertVnPriceToInt(total) * 100;
